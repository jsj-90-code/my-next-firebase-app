...
type Baseline = { id:string; label:string; revenue:number | null; hourlyRate:number | null };

function ScenarioInputs({ baseline, productRatio }: {baseline:Baseline; productRatio:number}) {
  const [pcRevenue, setPcRevenue] = useState(String(Math.round(baseline.revenue! * (1-productRatio))));
  const [productRevenue, setProductRevenue] = useState(String(baseline.revenue! - Math.round(baseline.revenue! * (1-productRatio))));
  ...
  return <div className="mt-3 space-y-3">
    <p className="text-xs leading-5 text-[#8a8072]">
      초기 매출은 현재 예측값을 설정상 먹거리 비중 {Math.round(productRatio*100)}%로 나눈 가정입니다.
    ...
    </div> : <p role="status" className="text-sm">매출과 변경요금은 0 이상, 기준요금은 0보다 큰 숫자를 입력하세요.</p>}
    <p className="text-xs leading-5 text-[#8a8072]">
...
export function PriceScenarioPanel({ baselines, productRatio }: {baselines:Baseline[]; productRatio:number}) {
  ...
  return <details className="app-card mt-3 rounded-xl p-4">
    ...
    <p className="mt-2 text-xs leading-5 text-[#8a8072]">주 예측값의 요금별 비교는 이용시간 고정 계산이 아닙니다. 같은 이용시간에서 요금만 바꿀 때는 아래에서 비교하세요.</p>
    ...
    <ScenarioInputs key={`${selected.id}:${selected.revenue}:${selected.hourlyRate}:${productRatio}`} baseline={selected} productRatio={productRatio} />
  </details>;
}
