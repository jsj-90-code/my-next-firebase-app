...
function buildPrompt(multiTile: boolean): string {
  return (
    ...
    "나란히 붙어있거나, 벽/통로로 분리된 섬 모양 덩어리 등). 통로, 벽, 카운터, 주방, 화장실, " +
    "창고처럼 좌석이 없는 공간은 절대로 구역 상자 안에 포함하지 마세요 — 다 그린 뒤에 각 구역 " +
    "상자 안을 다시 살펴서 이런 비좌석 공간이 섞여 들어간 게 있으면 경계를 좁히세요.\n" +
    "2. 두 줄이 통로를 사이에 두고 서로 마주보는 형태(마주보고 앉는 자리 배치)라면, 통로 " +
    "양쪽 두 줄과 그 사이 통로까지 통틀어 하나의 구역으로 잡으세요 — 손님이 그 통로에 서서 " +
    "봤을 때 한 구역으로 인식되게 하는 게 목적입니다. 마주보는 두 줄을 통로 기준으로 좌/우(또는 " +
    "상/하) 두 구역으로 쪼개지 마세요.\n" +
    ...
    "두꺼운 파티션으로 구분되어 있으면 그 부분만 따로 구역으로 뽑으세요).\n" +
