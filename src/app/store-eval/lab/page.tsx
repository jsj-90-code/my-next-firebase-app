          ...
          <div className="mt-1 rounded border border-[var(--sl-line)] p-2">
            ...
            <div className="mt-1">
              ...
              {p.specialDemandMultipliers["대학가"] === 1
                ? <> 사용자(2026-09-24): <b>&ldquo;호구포역·오송·구리돌다리 예상매출이 배수 없을 때가 더 적합했다.&rdquo;</b> 세 후보지가
                  산업단지·대학가·관광유흥 각각입니다 — 호구포역 41.3→49.5% · 오송 28.9→37.6% · 구리돌다리 23.8→33.3%(배수 1.0 → 켤 때).
                  배수는 수요에만 곱하고 점유율은 수요와 무관해서 <b>그 유형 후보지 가동률이 정확히 ×배수</b>가 됩니다 — 자사에서
                  &ldquo;평균 편향 0&rdquo;으로 고른 값이 후보지에선 검증 없이 통과합니다(`_labCandidate.test.ts` (10) 재고 표).
                  자사 40곳 비용은 MAE 5.21 → 5.81%p(유형별 편향: 대학가 5곳 −7.5 · 산업단지 3곳 −4.1 · 관광유흥 2곳 −8.1). 유형별 근거도
                  약했습니다 — 산업단지 3곳은 25%p 갈리고(1.0이어도 탕정역 +7.4), 관광유흥 2곳(수원인계·야당)은 층 가르기에서
                ...
                : <> 사용자 결정(2026-09-24 밤). 낮에 세 배수를 껐던 이유는 <b>호구포역(산업단지 ×1.2)·오송(대학가 ×1.3)·구리돌다리(관광유흥 ×1.4)
                  ...
                  <b>관광유흥도 밤 늦게 ×{p.specialDemandMultipliers["관광·유흥"]}(높음만)</b>로 켰습니다 — 우리 몫 상한(당시 80%, 2026-09-25 재측정 뒤 85%)을 넣은 뒤 다시 재니 자사
