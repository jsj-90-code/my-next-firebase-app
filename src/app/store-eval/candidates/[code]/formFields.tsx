...
export function InteriorScoringGuide() {
  return (
    <div className="app-card-sm col-span-full mt-3 rounded-lg px-3 py-2 text-xs leading-5 text-[#5c5346] dark:text-[#c9bfae]">
      <p>
        <strong>존구성</strong>(내부비중 50%) = <strong>룸종류점수 × 30% + 특화좌석점수 × 70%</strong>.
        아래 존 개수로 자동계산합니다.
      </p>
      <p className="mt-2">
        <strong>룸 종류</strong>는 1인룸 · 2인룸 · 팀룸 <strong>세 가지만</strong> 셉니다(퍼스트클래스존은
        팀룸과 같은 칸). 벽·문으로 분리된 것만 룸입니다 — 칸막이만 있으면 룸이 아니라 특화좌석이고,
        듀얼모니터만 있는 좌석도 1인룸이 아닙니다.{" "}
        <strong>특화좌석</strong>은 이름과 무관하게 실재하는 좌석을 전부 셉니다(VIP존은 1인석, 프렌즈존은
        좌석 수, 퍼스트클래스존은 룸 하나에 11석). 성인존·게이밍존처럼 이름만 나눈 구역은 미인정.
      </p>
      <p className="mt-2 text-[#8a7f6d] dark:text-[#a89c88]">
        ⚠️ <strong>2026-09-19에 잣대를 바꿨습니다.</strong> 전에는 &ldquo;존 <strong>이름</strong>이 몇
        가지냐&rdquo;로 다양성을 셌는데, VIP존·프렌즈존·퍼스트클래스존은 우리 브랜드 용어라 경쟁점
        조사 228건이 전부 0건이었습니다. 조사자가 경쟁점의 같은 실체(파티션 1인석 등)를 보고도 그 칸에
        안 넣기 때문입니다. 그래서 <strong>이름을 세면 자사가 항상 이겼습니다</strong>. 지금은 양쪽 다
        조사되는 물리 사실(룸이냐 아니냐 · 좌석이 몇 개냐)만 셉니다.
      </p>
      <p className="mt-2">
