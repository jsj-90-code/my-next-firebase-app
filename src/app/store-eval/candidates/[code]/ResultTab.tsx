...
export function ResultTab({ candidateCode }: { candidateCode: string }) {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      const evaluated = evaluateCandidate({ candidate, competitors, locationEvaluation, settings, existingMarketDemands });
    ...
    <div className="flex flex-col gap-6">
      ...
      <section className={sectionClass}>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <ResultCard
            label="V61 기본예측(참고)"
            value={formatWon(result.v61Baseline)}
            hint={result.v61IsFallback ? "폴백 근사치 (§4.1 회귀식, 07 Apps Script 값 아님)" : undefined}
          />
