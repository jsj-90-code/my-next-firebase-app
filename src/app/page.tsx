...
const TOOLS = [
  ...
  },
  {
    ...
    label: "점포평가 시스템",
    description:
      "신규후보지·경쟁점·입지동선을 입력하면 예상매출과 최종판정을 계산하고, 기존 가맹점 실매출로 모델을 검증합니다.",
    icon: <path d="M4 19V9l8-5 8 5v10 M4 19h16 M9 19v-6h6v6" />,
  },
  {
    // 2026-09-22 신설 — 주소만 넣는 초기평가. **점포평가 시스템과 완전히 분리된 독립 도구**로
    // 걸었다(사용자 지시: "링크를 완전히 분리해줘 v62웹에넣지말고"). 산식은 같은 운영 V62를
    // 부르지만 화면과 주소가 따로다 — 결재용 정밀 평가와 섞이지 않게 하려는 것이다.
    href: "/quick-eval",
    label: "주소만 초기평가",
    description:
      "주소 한 줄만 넣으면 인구·유동·경쟁점을 자동으로 모아 예상매출과 AI 평가문을 냅니다. 후보 선별용이고, 결재 숫자는 점포평가 시스템에서 냅니다.",
    icon: <path d="M12 21s7-6.2 7-11a7 7 0 1 0-14 0c0 4.8 7 11 7 11z M12 10.5h.01" />,
  },
  {
    // 2026-09-15 신설 — 교과서식 산식(수요 -> 점유율 -> 매출)을 손으로 돌려보는 실험 화면.
    // 운영 산식·저장값은 건드리지 않는다. 사용자 요청으로 홈에도 기존 도구들과 같이 건다.
    href: "/store-eval/lab",
    label: "점포평가 실험실",
    description:
