...
export function defaultModelSettings(): Omit<ModelSettings, "updatedAt" | "updatedBy"> {
  return {
    ...
    v61Training: {
      ...
      // 2026-09-15 0.06 → 0.09 (사용자 확정). 비중 재배분·접근성 곱과 함께 쟀을 때만 의미가 있다 —
      // 이것만 단독으로 올리면 ±20% 승률이 41%로 우연 수준이었다. 0.12 이상은 급격히 나빠진다.
      ...
      //   정가 1000원 3곳 112% · 1200원 9곳 101% · 1300원 8곳 97%
      //   정가 1500원 10곳  90% · 1700원 2곳  88% · 1800원 1곳 83%
      // 38곳 리브원아웃 검증: 지수 1.0(종전) MAPE 10.80%/±20% 86.8% → 0.546 MAPE 10.00%/±20% 92.1%.
      // ⚠️ 이 값은 **검증 정확도를 보고 고른 게 아니라** 실효단가 실측에서 독립적으로 나왔다
      // (정확도만 보면 0.30이 MAPE 9.65%로 더 낮지만 그건 답안지를 보고 고르는 것이라 안 쓴다).
      // ⚠️ 한계: R² 0.346이라 같은 1500원이라도 매장별로 72~108%까지 벌어진다. 그리고 **매장 간
      // 비교지 같은 매장의 요금 변경 전후가 아니다** — 어느 매장이든 요금을 실제로 바꾸면 그때
      // 직접 확인할 것. 표본이 늘면 재측정한다.
      tariffEffectiveExponent: 0.546,
      tariffReferenceRate: 1343,
    },
    ...
    // 2026-09-15 재배분 (사용자 확정). 하드 18.52 / 먹거리 16.67 / 시설 46.30 / 입지 18.52
    //                        → 하드 25 / 먹거리 5 / 시설 55 / 입지 15
    //
    // **바뀐 핵심은 먹거리다.** 사용자 지적("먹거리 평가 부분이 분별력이 좀 없으니까")을 재봤더니
    // 그대로였다 — **자사 38곳 전부 4.00점(표준편차 0.000, 값 1가지)**이라 매장 간 차이를 아예
    // 못 만든다. 경쟁점도 5단계(1.5/2/2.5/3/3.5)뿐이라 사양 98가지·시설 59가지보다 훨씬 거칠다.
    // 먹거리를 통째로 빼도 경쟁력 격차가 평균 -1.18%만 움직인다. 즉 16.67%가 **거의 상수에
    // 배분되면서 시설·입지의 영향력을 희석**시키고 있었다. 먹거리가 사업적으로 안 중요한 게
    // 아니라 **지금 평가 방식이 그걸 측정하지 못한다** — 자사 먹거리를 매장별로 평가할 수 있게
    // 되면 비중을 되돌려야 한다.
    //
    // 사양은 **올렸다**(18.52 → 25). 탐색만 보면 사양 5%가 MAPE 9.10%로 가장 낮았지만
    // 사용자 판단("사양 비중을 지금보다 내리는 건 현실감각·직관으로 어렵다, 오히려 높여야 한다")과
    // 데이터가 충돌하지 않았다 — 사양 25%는 MAPE 9.37%로 0.27%p 손해지만 **오차폭 39.4%p(사양
    // 5%안 42.2%p보다 좋음)·±10% 24곳·최악 20%로 오히려 낫다.** 사양 5%안은 38곳에 과하게
    // 맞춘 쪽이었다. 30%까지도 재현되지만(±10% 25곳) MAPE가 9.63%로 더 밀려서 25%로 정했다.
    // 35% 이상은 재현이 무너진다(승률 60% 아래).
    //
    // 검증(정식검증군 38곳 리브원아웃, 격차하한 0.09·접근성 곱과 함께):
    //   MAPE 10.00%→9.37% · 중앙 9.47%→8.52% · ±10% 20→24곳 · ±20% 35→37곳
    //   최악 25%→20% · 오차폭 47.2%p→39.4%p
    //   28곳 무작위 150회 재현: MAPE 87% 중앙 72% ±10% 74% ±20% 82% 폭 92% 최악 95% (여섯 다 60%↑)
    // 근거·재현: docs/demand-structure-search-2026-09-14.md
    competitivenessWeights: { spec: 0.25, food: 0.05, interior: 0.55, location: 0.15 },
