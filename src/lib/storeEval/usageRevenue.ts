 ...
 * outer LOOCV target is excluded again by the caller.  The full correction
 * outperformed the former half-shrink in repeated grouped hold-outs and in
 * the chronological hold-out, for both the PC-usage and product components.
 */
export function crossFittedCorrection(pairs: { predicted: number; actual: number }[]): number {
  ...
  for (const ratio of ratios) {
    ...
    if (cumulative >= halfWeight) return ratio.value;
  }
  return 1;
}
