 ...
 */
/**
 * ⚠️ **2026-09-14 되돌림 — 사용자 확정.** 아래 2026-09-10 기록은 그대로 둔다(왜 한 번 켰었는지가
 * 다음 사람에게 중요하다).
 *
 * 되돌린 이유(사용자): **"경쟁점 요금에 따라 우리 매출이 영향받는 구조가 아니고, 신규매장은
 * 상권 요금제에 맞춰 요금을 설정하기 때문에 요금 자체로 매출액이 평가되는 게 맞다."**
 * 실무에서 "요금을 1,500 → 1,000으로 내렸는데 매출이 7%밖에 안 빠진다"가 납득이 안 된다는
 * 지적에서 출발했다. 같은 이용자 기준이면 PC매출은 33% 빠지는 게 맞다.
 *
 * 되돌리면서 감수한 것(실측, 정식검증군 38곳):
 *   MAPE        9.88%  → 10.80%   (나빠짐)
 *   중앙값 오차  9.02%  →  9.30%   (나빠짐)
 *   ±10% 적중   52.63% → 52.63%   (동일)
 *   ±20% 적중   89.47% → 86.84%   (34곳 → 33곳)
 *   최악 매장   26.02% → 23.34%   (오히려 좋아짐)
 *
 * 후보지 9곳 영향: 대부분 ±2% 안, 최대 -4.8%(신중동점). 등급이 뒤집힐 크기는 아니다.
 * 요금이 싼 매장이 내려가고 비싼 매장이 조금 오른다 — 요금이 직접 반영되니 당연한 방향이다.
 *
 * 다시 켜려면 이 값을 "usage"로 바꾸면 된다(검증·후보지 예측이 같은 값을 쓴다).
 */
