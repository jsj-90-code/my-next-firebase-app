...
// 이 값은 후보지 결과 탭을 열 때, 그리고 2026-09-26부터 **매일 06:00 크론**(dailyRecompute.ts)이 다시
// 계산해 저장한다. 크론도 이 함수로 "배지가 켜질 후보지"를 골라 다시 쓰므로 배지는 하루 안에 꺼진다.
// (그 전에는 결과 탭을 안 열면 옛 숫자가 계속 사실처럼 떠 있었다.)
//
...
export function freshnessHint(f: Freshness): string | null {
  if (f.state === "결과없음") return "아직 계산된 결과가 없습니다. 매일 06:00에 자동으로 계산되고, 바로 보려면 결과 탭을 한 번 여세요.";
  if (f.state === "최신") return null;
  return `${f.reasons.join("·")}이(가) 마지막 계산 이후에 바뀌었습니다. 매일 06:00에 자동으로 다시 계산되고, 바로 보려면 결과 탭을 한 번 여세요.`;
}
