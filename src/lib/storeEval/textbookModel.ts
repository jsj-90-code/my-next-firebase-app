...
export type TextbookParams = {
   ...
   *
   * 기본값은 2026-09-16 측정값이다. 필요 점유율(= 실측가동률 ÷ 경쟁없을때 예측)을 유형별로
   * 보고 "없음" 대비 배수를 잡았다 — 필요 점유율이 100%를 넘은 4곳 중 2곳이 군부대였다:
   *
   *   군부대   2곳 118.3% -> 2.25배      대학가   5곳 76.0% -> 1.45배
   *   산업단지 3곳  72.8% -> 1.39배      기타     4곳 65.6% -> 1.25배
   *   없음    22곳  52.5% -> 1.00        관광·유흥 2곳 44.1% -> 0.84배
   *
   * ⚠️ **표본이 2~5곳이라 계수를 확정할 수준이 아니다.** 방향(군부대 > 대학가 ≈ 산업단지 >
   *    없음)은 명확하지만 값은 흔들릴 수 있어 화면 조절판으로 빼 뒀다.
   ...
   * ⚠️ **축척이 순환한다 — 2차 가공 때 꼭 볼 것.** 수요 축척 A는 독점 3곳에서 정하는데 그중
   *    **2곳이 특수수요를 갖고 있다**(탕정역=산업단지/보통, 남악=기타/보통, 광주각화=없음).
   *    배수를 넣으면 그 2곳의 수요가 커져 A가 작아지고, "없음" 매장들이 전부 밀려 올라간다.
   *    순수 기준점은 광주각화점 하나뿐이다. 실제로 1차 적용 결과:
