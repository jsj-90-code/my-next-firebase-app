...
export type TextbookParams = {
   ...
   *    찾기로 했다. 수준 오차는 관측된 기전(`outsideOptionIp` 20대)이 대신 받는다.
   *    남은 건 **퍼짐 1.73배 과장 하나**이고, 그게 다음 과녁이다.
...
export const DEFAULT_TEXTBOOK_PARAMS: TextbookParams = {
  ...
  gapExponent: 4.0,
  // ✅ **2026-09-21 밤 채택 — 20대.** 자유 계수가 아니라 **독점매장에서 관측한 값**이다.
  // 축척을 원장 실측으로 못 박으니(위) 독점 3곳에서 날 수요식이 일제히 19% 과대였고,
  // 점유율이 1인 곳이라 그 초과분이 곧 "아무 PC방에도 안 가는 몫"이다. 매장별로 직접 나온다:
  //   탕정역 20.9대 · 광주각화 19.8대 · 남악 19.3대 → 기하평균 **20.0대**
  //
  // ⭐ **기전 판정을 통과했다.** 같은 19%를 "수요 추정이 19% 많다"로 볼 수도 있는데, 그러면
  //    전 매장 수요를 0.84배 해야 한다. 두 해석은 독점 3곳에서 **똑같은 값**을 내지만
  //    경쟁 있는 35곳에서 갈린다 — 수요를 깎으면 그 35곳이 **−3.75%p로 일제히 과소예측**이
  //    되고, 외부옵션으로 넣으면 −1.43%p에 그친다(MAE도 7.05 vs 6.56%p). 경쟁 35곳 MAE는
  //    비례형(자사PC×0.189)보다 절대형이 낫다(6.56 vs 7.11%p) — Huff 교과서 형태와도 같다.
  //    → **수요 추정은 안 틀렸다. 19%는 경쟁이 없는 곳에만 걸리는 몫이다.**
  // 측정: `_shareDiscrimination.test.ts` [B5] · `_scaleMeasured.test.ts` [측정 1]
  // ⚠️ 독점 3곳에서 나온 값이다. 표본이 늘면 다시 잰다.
  outsideOptionIp: 20,
  ...
  //    말고 기전을 찾자"라서 껐다. 대신 b가 삼키던 수준 오차를 **관측된 기전**으로 받는다
  //    (outsideOptionIp 20대 — 바로 위 주석). 되살리려면 아래 한 줄을 되돌리면 된다.
  //
