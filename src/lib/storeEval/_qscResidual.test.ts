...
//
// ① **운영 V62의 잔차를 설명한다 — 날것 상관보다 강하게.** (n=29 · 유의선 0.371)
...
//
// ② **"관리가 매출에 얼마"** — 탄력도 **0.84**. QSC 1% 높으면 매출 0.84% 높다.
...
//
// ③ **앞 항을 반영할수록 커진다 — 편심도와 정반대다.** 실험실 점유율 잔차(n=23):
...
//
// ④ **ψ 관문을 통과한다.** 점유율 = [품질] x (층수자÷3.42)^0.25 x (중심도÷3.22)^0.25 x **(QSC÷92.6)^ψ**
...
//
// ⑤ **기존 계수는 안 흔들린다 — "관리 교란에서 풀려남"은 일어나지 않았다.**
...
//
// ⑥ **경쟁력점수의 그림자가 아니다.** QSC ↔ 경쟁력점수 r=0.385로 겹치는데, 정체는 **스펙**이다
