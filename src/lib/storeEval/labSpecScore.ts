...
//
// ── GPU와 CPU 둘 다 켰다 (2026-09-18) ─────────────────────────────────────
//   GPU  성능지수 + 세대 벌점 0.15   r 0.563 -> 0.581   대조군 p=0.013 ✅
//   CPU  성능지수 + 세대 벌점 0.30   r 0.581 -> 0.585   대조군 p=0.213 ❌ (아래 참고)
//
// GPU 대조군은 "델타의 크기는 그대로 두고 **어느 모델에 붙는지만** 뒤섞기" 300회다. 뒤섞으면
// r 개선폭이 95퍼센타일 0.014인데 실제 표는 0.018이다. 즉 이 표가 좋아진 건 점수를
// 움직였기 때문이 아니라 **맞는 모델에 맞게 움직였기** 때문이다.
//
// ── ⭐ CPU는 대조군을 못 넘었는데 왜 켜나 — 서술과 영향력은 다른 것이다 ────
// 하루 동안 CPU를 세 번 재고 두 번 껐다가 켰다. 헷갈린 이유는 내가 **두 가지를 섞어서**다.
// 사용자가 그 둘을 갈라줬다(2026-09-18):
//
//   *"아니 영향력은 잘모르겠다지만 CPU사양별 차이는 있잖아."*
//   *"제품스펙을 고객이몰라도 이용했을때 프레임이나 버벅임 이런것들은 느낄수도있잖아.
//     CPU가 거기에 영향을 끼치잖아"*
//
//   **영향력** — 매출에 얼마나 미치나  -> **비중**이 담당한다 (하드웨어 내부 0.2 = 경쟁력의 5.3%)
//   **사양 차이** — 하드웨어가 실제로 다른가 -> **변환표**가 담당한다
//
// `i3 13100F = i5 13400F`는 영향력이 작아서 같은 게 아니라 **그냥 틀린 서술**이다. 그리고
// 변환표를 고치는 건 CPU에 무게를 더 싣는 게 아니다 — 무게는 비중이 정하고, 변환표는 **같은
// 무게 안에서 더 맞게 배분**할 뿐이다.
//
// 이 구분이 대조군 해석도 정리해준다. **대조군은 "영향력"을 재는 도구다.** 비중 5.3%짜리
// 항목이 못 넘는 건 당연하다(벌점 0.075~0.7 전부 p=0.21~0.31). 우리가 고치는 건 영향력이
// 아니라 서술이라, 대조군이 판정할 대상이 아니다. [[feedback_item_vs_coefficient]]의
// "검정 결과로 항목을 자르지 말 것"과 같은 자리다.
//
// ⚠️ 그래도 **성적이 크게 나빠지면 멈춘다**는 안전선은 둔다. 여기 대가는 MAPE 22.59% ->
//    22.85%(+0.26%p)이고 r은 오히려 0.581 -> 0.585다. 존구성이 +0.58%p를 내고 뜻을 산 선례가 있다.
//
// ── 경로가 있다 — 손님이 사양표를 몰라도 버벅임은 느낀다 ──────────────────
//   CPU 사양 -> 프레임·버벅임 -> 손님 체감 -> 재방문
// 손님이 "i5 12400F"를 알아볼 필요가 없다. 그리고 구형의 저하는 **평균 프레임보다 1% 로우
// (순간 버벅임)에서 크게** 나타나는데, 성능지수는 평균만 담는다. 그 몫을 세대 벌점이 받는다.
// 벌점을 0.3으로 둔 이유가 이것이다 — 연 단위로 GPU와 맞춘 0.075는 i7 10700K(2020년)를
// 3.00점으로 만들어 i5 13400F(2022년)와 같아진다. 현장 감각에 안 맞는다.
//
...
//
// ── 세대 차등의 두 번째 경로 — 손님이 세대는 안다 ─────────────────────────
// 사용자(2026-09-18): *"세대별 기본적인 성능우위는 일반고객도 알수있는 정보니까 세대별
// 차등도 적용하는쪽으로"*
// 그래서 세대 차등은 경로가 **둘**이다 — 체감(위 프레임·버벅임)과 **인지**(세대 숫자는
// 사양표만 봐도 일반 고객이 대략 안다). 티어(i3/i5/i7)보다 세대가 더 널리 읽히는 정보다.
//
// ── ⚠️ 한 번 헛다리를 짚었다 (근거 보존) ──────────────────────────────────
// 중간에 내가 "PC방에서 CPU 차이는 잘 모르겠다"는 말을 **서술까지 대충 둬도 된다**는 뜻으로
// 읽고 "안 켠다"로 결론 낸 적이 있다. 틀렸다. 그 말은 **영향력**에 대한 것이고, 영향력은
// 비중이 담당한다(위 ⭐ 참고). 같은 실수를 반복하지 않도록 남겨 둔다.
//
// ── 아래는 그 전에 재 본 것들 (근거 보존) ─────────────────────────────────
// ── i3 매장이 매출이 낮은 건 CPU 탓이 아니다 — 코호트 교란 ────────────────
...
//
// ⚠️ **이건 "티어를 넣지 마라"는 뜻이 아니다.** i3 9곳의 낮은 매출을 *티어 반영의 근거로
//    쓰면 안 된다*는 뜻이다(그건 코호트가 만든 것이다). 티어를 넣는 근거는 따로다 —
//    **하드웨어가 실제로 다르다.** 그 둘을 섞지 말 것.
//
// 참고로 당시 i3를 고른 판단도 자료가 지지한다 — 성능지수로 **i3 13100F 82 · i5 12400F 88**이다.
// 7% 차이라 "크게 차이 안 난다"고 본 게 합리적이었다(4코어지만 클럭이 높아 e스포츠에선 근소).
//
// ── 사용자 질문 셋에 대한 답 (2026-09-18) ──────────────────────────────────
// 1. *"4070 4080은 성능대비 5060보다 우위일텐데 이런거 어케할지?"*
//    성능지수로 옮겨서 풀었다. RTX 4070은 3.50 -> 4.43으로 RTX 5060(4.00) 위에 선다.
//    처음엔 상한 5에 5070·4070·3080·5080이 다 몰렸는데, 위쪽 기울기(0.45)와 세대 벌점으로
//    풀었다(위 LAB_PERF_LOG_STEP_UP · LAB_GEN_PENALTY 참고).
...
//    생산성에선 앞서지만 PC방은 게임이 전부다. 성능지수 102 대 100에 세대가 하나 위라
...
/**
 * **CPU 성능지수표를 쓸지.** 기본 `true` (2026-09-18 채택 — 파일 위 ⭐ 참고).
 *
 * 끄면 운영 세대 산술(`세대 - 10`)로 돌아간다. 대가는 MAPE 22.59% -> 22.85%(+0.26%p)이고
 * r은 0.581 -> 0.585다. **대조군은 못 넘었다(p=0.213)** — 켠 근거는 영향력이 아니라
 * **서술의 정확성**이다(`i3 13100F = i5 13400F`가 틀린 서술이라서).
 */
export const LAB_USE_CPU_PERF_INDEX = true;
