 ...
 *
 * 2026-09-03 재보정 (간략 2.0 / 외관만 1.5 → 둘 다 2.5). 근거 2가지:
 *
 * 1) **옛 1.5는 실측 분포 밖이었다.** 상세조사 경쟁점 154곳의 실제 종합점수 분포는
 *    최소 1.63 / 25%ile 2.46 / 중앙 2.69 / 평균 2.72 / 최대 3.71로, **1.5점 이하가 0곳**이다.
 *    즉 "조사 안 한 경쟁점 = 실측된 어느 경쟁점보다도 약하다"고 가정하는 셈이라 우리 경쟁력격차와
 *    예상매출이 계통적으로 부풀려졌다. 2026-09-02에 미조사 경쟁점 PC대수 기본값을 70→90
 *    (실측 194곳의 25%ile)으로 고친 것과 **완전히 같은 종류의 과소추정**이었다.
 *    새 값 2.5는 그 전례대로 실측 25%ile(2.46)을 반올림한 것이다.
 *
 * 2) **간략/외관만 두 단계로 나눌 실익이 없었다(사용자 질문으로 확인).** 조사수준별 실측
 *    보유율을 재보니 — 간략 42곳은 먹거리 95%·인테리어 95%·관리 83%·층수 90%로 대부분
 *    실측이 채워져 있어 기본값이 쓰이는 곳이 거의 VGA뿐이고(종합점수도 14종으로 분산),
...
/**
 * 종합 경쟁력점수 = settings.competitivenessWeights로 가중합한다. **여기에 숫자를 적지 않는다**
 * — 2026-09-03에 하드웨어:시설:입지가 30:40:30 → 20:50:20으로 바뀌었는데 이 주석과 설명 화면이
 * 옛 값을 달고 있었다(2026-09-14 확인). 지금 값은 settings.ts를 보라.
 ...
 *
 * ── 천장 높이의 근거 (2026-09-23, quickEvalDefaults.QUICK_EVAL_DEMAND_CEILING 주석에 스윕 표) ──────
 * 기존점의 실제 PC 이용시간(PC매출 ÷ 요금) ÷ 자사수요(상권수요 × 점유율)는 **4~43시간**, 중앙값
 * 15시간이다(문경시청점 68h는 수요 측정이 틀린 매장이라 사용자 결정으로 채점에서 뺐다). 높이는
 * 실측 최대치가 아니라 **되짚기 스윕**으로 골랐다 — 천장은 **예측** 이용시간을 보므로, 실측이
 * 천장을 넘는 매장도 예측이 낮으면 안 걸린다. 걸린 기존점이 전부 실제 매출에 가까워지는 가장 낮은
 * 값이 채택값이다. 이 숫자가 "1명이 그만큼 쓴다"는 뜻은 아니다 — V62의 상권수요가 실제 손님 수를
 * 몇 배 작게 잡는다는 뜻이다(`project_pingbot_validates_demand`).
 *
 * ⚠️ 운영 V62는 기본 **꺼짐**(settings.demandCeilingHoursPerUser = null). 주소만 초기평가만 켠다.
 * ⚠️ 채택 조건은 "걸린 기존점이 전부 실제에 가까워진다"이고 `_quickEvalBias.test.ts` "수요 천장"이 지킨다.
