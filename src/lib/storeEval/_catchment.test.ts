...
//
// ⚠️⚠️ **2026-09-20 — 이 자료에 폐업 매장이 섞여 있다. 아래 결론을 근거로 쓰지 말 것.**
//    사용자가 문경시청점을 지도로 대조하니 자료 7곳 중 **3곳(43%)이 폐업**이었다
//    (팡팡피씨방·스위티 피시방·라이또 PC방 — 전부 1.5km 지점). 카카오 장소 API는 폐업
//    표시를 따로 주지 않는다.
//
//    그래서 이 파일이 세는 **"대체 PC방 수"와 "최단 대체재 거리"가 둘 다 부풀려져 있고**,
//    그 위에서 내린 **기각 결론(관문 1 미달)도 다시 봐야 한다.** 우리 정식 경쟁점 DB는
//    500m까지만 다루므로, 오염 구간(500m~2km)이 정확히 이 가설이 쓰는 구간이다.
//
//    다시 열려면 54곳 전체를 사람이 지도로 대조하거나, 폐업 여부를 주는 출처(공공데이터
//    인허가 정보 등)로 갈아끼워야 한다. 상세: docs/data-issues.md 2026-09-20 항목.
//
// 실행:
