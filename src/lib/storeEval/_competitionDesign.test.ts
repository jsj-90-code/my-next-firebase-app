...
//
// 2) **상권 겹침(거리 감쇠)을 넣었다.** 이게 지금 산식에 없던 것이고, gamma=4가 대신
//    떠맡고 있던 일이다. 경쟁점 거리는 225/225 전수로 있다(중앙 135m, 최대 1500m).
//
//      점유율 = 자사PC x 점수^θ ÷ (자사PC x 점수^θ + Σ 경쟁PC x 점수^θ x exp(-거리/d0))
//
// ── 결과 (경쟁상권 실측 점유율 대비) ────────────────────────────────────
//   겹침없음 θ=0    편향 -64.7% · MAPE 64.7% · r=0.157   <- 지금 구조의 민낯
//   겹침없음 θ=4    편향 +10.4% · MAPE 33.8% · r=0.372   <- 지금 기본값(gamma=4)
//   d0=150  θ=0    편향 -33.5% · MAPE 41.5% · r=0.442   <- 거리만으로 r이 3배
//   d0=250  θ=2    편향  -6.0% · MAPE 29.2% · r=0.491   <- 최선
//   d0=150  θ=2    편향  +7.3% · MAPE 30.2% · r=0.510
//
// **거리 감쇠만 넣어도 r이 0.157 -> 0.442로 뛴다**(경쟁력을 하나도 안 쓰고). 거기에 θ=2를
// 더하면 0.510이다. 편향도 -64.7% -> -6.0%로 거의 해소돼 gamma=4라는 반창고가 필요 없어진다.
//
