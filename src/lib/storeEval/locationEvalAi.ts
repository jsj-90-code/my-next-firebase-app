...
const FIELD_SCHEMAS: Record<LocationEvalFieldKey, ReturnType<typeof scoreProp> | ReturnType<typeof enumProp> | ReturnType<typeof textProp>> = {
  ...
  specialDemandType: enumProp(
    ...
    "특수수요유형 — 인구통계에 안 잡히는 이용자를 데려오는 수요원. '대학가'는 학부 중심 종합대학(재학생 1만 명 이상)의 " +
      ...
      "행정타운, 전문대 소규모 캠퍼스는 대학가가 아니라 '기타' 또는 '없음'. '군부대'는 부대 정문 생활권(외출·외박 병사가 걸어오는 " +
      "거리). '산업단지'는 공장 근로자 상권(생산직 교대 근무). '관광유흥'은 술집·모텔 밀집 유흥가나 관광지.",
  ),
  specialDemandIntensity: enumProp(
    ...
    "특수수요강도 — '높음'은 그 수요원이 이 상권의 주된 손님층일 때만(대학가: 위 조건을 전부 충족하고 학생이 주 고객). " +
      ...
      "군부대 '높음'은 부대 정문 생활권으로 외출·외박 병사가 주 고객일 때. " +
      "관광유흥 '높음'은 술집·모텔·클럽이 밀집한 유흥가 한복판이거나 관광지라 밤 손님·외지 방문객이 주 고객일 때(유흥가 언저리·아파트 상가는 '보통'). " +
      "산식은 대학가·산업단지·군부대·관광유흥 배수를 '높음'에만 곱하므로, 확실하지 않으면 '보통'으로 둘 것.",
  ),
...
const SYSTEM_PROMPT =
  ...
  "특히 '대학가/높음'은 학부 중심 종합대학 정문·후문 도보권에만. 약대·연구단지·행정타운은 대학가가 아니다)\n" +
  "- 외부유입제한(없음/보통/강함)\n" +
  "- 상권구조메모(자유서술)\n\n" +
