...
1. **V61 기본예측은 실제 릿지회귀 학습모형을 씁니다.** 원본은 비음수 릿지회귀+대당월매출 중앙값 블렌드라는 통계모형을 쓰는데, 이 알고리즘을 `src/lib/storeEval/calc.ts`에 정확히 포팅했고(`fitEmpiricalRevenueModel`/`predictEmpiricalRevenue`), 기존 가맹점의 실측 특징치(요금·IP당수요·경쟁력점수·실제매출)로 실제 학습시킵니다(표본은 계속 는다). `scripts/seedExistingStoreTrainingData.mjs`로 Firestore에 이 데이터를 넣어야 폴백 대신 이 실측 모형이 적용됩니다(안 넣으면 화면에 "임시 근사치·검증 전"이라고 표시됩니다). ⚠️ **정확도 수치를 이 문서에 적지 않는다** — 표본이 늘 때마다 바뀌어서 반드시 낡는다(2026-09-14에 실제로 여기 26곳·12.25%라는 2026-08 값이 남아 있었다). **지금 값은 앱의 `기존 가맹점 검증` 화면**이나 `storeEvalSystemStatus/accuracy` 문서를 보라. 참고로 2026-09-14 기준은 38곳·MAPE 9.88%·중앙값 9.02%·±20% 89.47%다. 6. 기존 가맹점 검증 화면에서 12개월 미완료 매장까지 포함한 재직기간별 코호트 검증 결과를 확인할 수 있습니다.
